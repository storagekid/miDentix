...
class ClinicPosterPriorityExports implements FromView
{
    ...
    {
        $campaign = is_array(request('campaign')) ? request('campaign')['id'] : null;
            ...
            foreach ($priorities as $priority => $sizes) {
                foreach ($sizes as $size => $types) {
                    ...
                    if ($types->has('Ext') && $types->has('Int')) {
                        ...
                        $types->forget('Int');
                        // $grouped[$clinicId][$priority][$size]['Normal'][] = $types['Ext'];
                        // $grouped[$clinicId][$priority][$size]['Normal'][] = $types['Int'];
                        // $grouped[$clinicId][$priority][$size]['Normal'] = count($types['Ext']) + count($types['Int']);
