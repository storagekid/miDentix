...
class StationaryCustomizablePrinter extends DentixPdfPrinter
{
    public $fileName, $pathToFile;
    public $clinic, $stationary;
    private $marginTop = 10;
    private $marginLeft = 10;
    private $marginRight = 10;
    private $marginBottom = 10;

    public function __construct($clinic, $stationary) {
        parent::__construct();
    ...
    public function makeStationary() {
      $this->fileName = $this->stationary->description . ' ' . $this->clinic->cleanName . '.pdf';
      $this->pathToFile =  storage_path('app/' . $this->directory . $this->fileName);

      switch ($this->stationary->name) {
        case 'A4_Dentix_Paper':
          return $this->makeA4();
          break;
        case 'A5_Dentix_Recepi':
          return $this->makeRecepi();
          break;
        case '225x115_Envelope':
          return $this->makeEnvelope();
          break;
        case 'C4_Envelope':
          return $this->makeC4Envelope();
          break;
        case 'businessCardClinic':
          return $this->makeClinicBusinessCard();
          break;
      }
    }
    ...
    public function makeA4() {
      ...
      $this->pdf->AddPage('P', $size);  // Simplyflied

      $this->pdf->ImageEps($this->logo512, 12.2, 11.1, 55, '', '', true, '', '', 0, false);
      ...
      $this->pdf->ImageEps($this->logo512_10, 30.7, 138, 148.7, '', '', true, '', '', 0, false);

      $this->pdf->SetFillSpotColor($this->Color512, 10);
      $this->pdf->RoundedRect($this->marginLeft, 273.5, 185.6, 15, 4, '1111', 'F');

      $this->pdf->setY(277);
      $text = $this->clinic->real_address->address_line_1 . '. ' . $this->clinic->postal_code . ' ' . $this->clinic->city . '. Tel. ' . $this->clinic->real_phone->number . '.';
    ...
    public function makeRecepi() {
      ...
      $this->pdf->ImageEps($this->logo512_10, 30.7, 64, 148.7, '', '', true, '', '', 0, false);
    ...
    public function makeEnvelope() {
      ...
      $this->pdf->ImageEps($this->logoClaim512, 152.7, 88.3, 67.3, '', '', true, '', '', 0, false);

      $this->pdf->SetFillSpotColor($this->Color512,10);
      $this->pdf->RoundedRect(5,83,141.6,26.25,4,'1111','F');
    
      $this->pdf->setY(86.5);
      $text = $this->clinic->real_address->address_line_1 . ".";
      ...
      $this->pdf->setY(94.5);
      $text = "Tel. ".$this->clinic->real_phone->number . ".";
      $this->pdf->SetFont($this->fontDxLight,'',9,'',false);
      $this->pdf->Cell(131,'',$text,0,0,'L',false,'');
    ...
    public function makeC4Envelope() {
        ...
        $this->pdf->ImageEps($this->logoClaim512, 220, 192, 97, '', '', true, '', '', 0, false);
    ...
    public function makeClinicBusinessCard() {
      ...
      $line1 = $this->clinic->real_address->address_line_1 . ", " . $this->clinic->postal_code . " " . $this->clinic->city;
      $line2 = "";
      $email = false;
      $domain = 'dentix.es';
      ...
      $this->pdf->ImageEps($this->logoWhite, 22+$bleed+$slug, 24+$bleed+$slug, 43, '', '', true, '', '', 0, false);
